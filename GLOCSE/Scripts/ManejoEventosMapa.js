    ...
    CajaDeBusqueda.addListener('places_changed', function ()
    {
        if (Rectangulo == null)
        {
            ...
            else
            {
                ...
                var CadenaHTML = '<strong style="  text-align: center;">Ubicación: </strong>' + Lugares[0].name + '<br><strong>Dirección: </strong>' + Lugares[0].formatted_address + '<br><strong>Latitud: </strong>' + Lugares[0].geometry.location.lat() + '<br><strong>Longitud: </strong>' + Lugares[0].geometry.location.lng();
...
function RestaurarEspaciosDeTrabajo(ID)
{
    ...
    Proyecciones[ProyeccionActiva].MarcadoresCollecion.forEach(function (Instancia)
    {
        ...
        google.maps.event.addListener(Instancia.Marcador, 'dragend', function (event) {
            ...
            $.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + Marcador.getPosition().lat() + ",+" + Marcador.getPosition().lng() + "&key="+ API_KEY, GeolocalizacionDrag);
        });
    ...
    if (Puntos.length == 2) {
        if (isNaN(Puntos[0]) == false && isNaN(Puntos[1]) == false)
        {
            ...
            if ((Puntos[0] > 0 & Puntos[0] < MapaCanvas.getDiv().offsetWidth) & (Puntos[1] > 0 & Puntos[1] < MapaCanvas.getDiv().offsetHeight)) {
                $.Notification.autoHideNotify('success', 'top right', 'Se han actualizado las coordenadas del marcador.');
                ...
                MapaAImagen(ProyeccionActiva, 0);
                    ...
                    $.Notification.autoHideNotify('warning', 'top right', 'Las coordenadas exceden el tamaño del mapa.');
            }
        }
        else {
...
function VerMarcadorMapa(Consumidor)
{
    ...
    if (InstanciaMarcador != null) {
        ...
        $.Notification.autoHideNotify('error', 'top right', 'No se puede ver el marcador en el mapa porque no existe aun')                                                ;
    }
}
...
function ActualizarLeyenda() {
    ...
    if (MostrarTodasProyecciones == false) {
        if (Proyecciones[ProyeccionActiva].ElipseDibujada == true) {
            if (Proyecciones[ProyeccionActiva].Opciones.MostrarElipse)
                if (ProyeccionActiva == 0) {
                    LeyendaItem.push({ Nombre: 'Elipse (' + Fecha.getFullYear() + ')', Icono: "https://k60.kn3.net/2/7/F/E/2/3/DEF.png" });
                }
                else if (ProyeccionActiva == 1) {
                    LeyendaItem.push({ Nombre: 'Elipse (' + (Fecha.getFullYear() + 10) + ')', Icono: "https://k60.kn3.net/E/2/F/1/4/4/7D6.png" });
                }
                else {
                    LeyendaItem.push({ Nombre: 'Elipse (' + (Fecha.getFullYear() + 20) + ')', Icono: "https://k60.kn3.net/9/8/7/D/9/E/2A1.png" });
                }
        }
        if (Proyecciones[ProyeccionActiva].Opciones.MostrarCentrosEventuales && Proyecciones[ProyeccionActiva].ElipseDibujada) {
            LeyendaItem.push({ Nombre: 'Centro Eventual (P' + (ProyeccionActiva + 1) + ')', Icono: "https://k61.kn3.net/5/0/2/8/4/C/383.png" });
        }
        if (Proyecciones[ProyeccionActiva].Opciones.MostrarElipse && Proyecciones[ProyeccionActiva].ElipseDibujada) {
            LeyendaItem.push({ Nombre: 'Subestación Electrica (P' + (ProyeccionActiva + 1) + ')', Icono: URLCentroElipse(ProyeccionActiva) });
        }
        if (Proyecciones[ProyeccionActiva].Opciones.MostrarConsumidores) {
            if (ProyeccionActiva == 0) {
                LeyendaItem.push({ Nombre: 'Consumidor (P1)', Icono: DevolverURLMarcador("A1") });
            }
            else if (ProyeccionActiva == 1) {
                LeyendaItem.push({ Nombre: 'Consumidor (P2)', Icono: DevolverURLMarcador("A2") });
            }
            else {
                LeyendaItem.push({ Nombre: 'Consumidor (P3)', Icono: DevolverURLMarcador("A3") });
            }
        }
    }
    else {
        ...
        if (OpcionesGlobales[0].TodosCentrosCarga == true) {
            ...
            LeyendaItem.push({ Nombre: 'Centros eventuales', Icono: "https://k61.kn3.net/5/0/2/8/4/C/383.png" });
        }
        if (!(OpcionesGlobales[0].TodosCentrosCarga == true || OpcionesGlobales[0].TodosCentrosEventuales == true)) {
            if (OpcionesGlobales[0].TodasElipses == true) {
                if (CentroInterseccion != null) {
                    LeyendaItem.push({ Nombre: 'Intersección', Icono: "https://k61.kn3.net/7/E/D/B/0/0/E14.png" });
                    LeyendaItem.push({ Nombre: 'Centro Optimo', Icono: URLCentroElipse(3) });
                }
            }
        }
    }
...
function UbicarPinPrueba() {
    ...
    google.maps.event.addListener(MarcadorPruebas, 'drag', function (event) {
        document.getElementById('LatitudMP').innerHTML = "Latitud: " + MarcadorPruebas.getPosition().lat();
        document.getElementById('LongitudMP').innerHTML = "Longitud: " + MarcadorPruebas.getPosition().lng();
        ...
        document.getElementById("IntersectaP1MP").innerHTML = 'Intersecta Con elipse P1 : ' + PuntoDentroPoligono(MarcadorPruebas.getPosition(), Proyecciones[0].Elipse);
        document.getElementById("IntersectaP2MP").innerHTML = 'Intersecta Con elipse P2 : ' + PuntoDentroPoligono(MarcadorPruebas.getPosition(), Proyecciones[1].Elipse);
        document.getElementById("IntersectaP3MP").innerHTML = 'Intersecta Con elipse P3 : ' + PuntoDentroPoligono(MarcadorPruebas.getPosition(), Proyecciones[2].Elipse);
        if (MostrarTodasProyecciones == false) {
            document.getElementById("OptimaMP").innerHTML = 'Esta en zona optima : ' + PuntoDentroPoligono(MarcadorPruebas.getPosition(), Proyecciones[ProyeccionActiva].Elipse);
        }
        else {
            if (SegundaProyeccionActivada == false) {
                ...
                if (PuntoDentroPoligono(MarcadorPruebas.getPosition(), Proyecciones[0].Elipse) == "Si" && PuntoDentroPoligono(MarcadorPruebas.getPosition(), Proyecciones[1].Elipse) == "Si") {
                    document.getElementById("OptimaMP").innerHTML = 'Esta en zona optima : Si';
                }
                else {
                    document.getElementById("OptimaMP").innerHTML = 'Esta en zona optima : No';
                }
            }
            else {
...
function ActualizarInformacionProyeccionActual()
{
    ...
    if(Proyecciones[ProyeccionActiva].Elipse != null)
    {
        ...
        document.getElementById('CElipseLongitud').innerHTML = 'Centro Elipse (Longitud) : '+ Proyecciones[ProyeccionActiva].Centro.getPosition().lng();
...
/*  document.getElementById('PActiva').innerHTML = "Proyección Activa: P" + (ProyeccionActiva + 1);
    ...
    if (ElipseP1Datos != "?") {
        document.getElementById('PCentroP1').innerHTML = "Elipse Centro P1 : (" + (ElipseP1Datos.Latitud) + "," + ElipseP1Datos.Longitud + ") , (" + ElipseP1Datos.X + "," + ElipseP1Datos.Y + ")";
        document.getElementById('PAreaP1').innerHTML = "Elipse Area P1 : " + google.maps.geometry.spherical.computeArea(Proyecciones[0].Elipse.getPath()) + " Metros Cuadrados";
    }
    else {
        document.getElementById('PCentroP1').innerHTML = "Elipse Centro P1 : ?";
        document.getElementById('PAreaP1').innerHTML = "Elipse Area P1 : ?"
    }


    ElipseP1Datos = CentroElipse(1);
    if (ElipseP1Datos != "?") {
        document.getElementById('PCentroP2').innerHTML = "Elipse Centro P2 : (" + (ElipseP1Datos.Latitud) + "," + ElipseP1Datos.Longitud + ") , (" + ElipseP1Datos.X + "," + ElipseP1Datos.Y + ")";
        document.getElementById('PAreaP2').innerHTML = "Elipse Area P2 : " + google.maps.geometry.spherical.computeArea(Proyecciones[1].Elipse.getPath()) + " Metros Cuadrados";
    }
    else {
        document.getElementById('PCentroP2').innerHTML = "Elipse Centro P2 : ?";
        document.getElementById('PAreaP2').innerHTML = "Elipse Area P2 : ?"
    }


    ElipseP1Datos = CentroElipse(2);
    if (ElipseP1Datos != "?") {
        document.getElementById('PCentroP3').innerHTML = "Elipse Centro P3 : (" + (ElipseP1Datos.Latitud) + "," + ElipseP1Datos.Longitud + ") , (" + ElipseP1Datos.X + "," + ElipseP1Datos.Y + ")";
        document.getElementById('PAreaP3').innerHTML = "Elipse Area P3 : " + google.maps.geometry.spherical.computeArea(Proyecciones[2].Elipse.getPath()) + " Metros Cuadrados";
    }
    else {
