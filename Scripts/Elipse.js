...
function CentrosEventualesDeCarga(Parametro, ProyeccionActual) {
    ...
    for (var I = 0; I <= 23; I++) {
        ...
        if (ProyeccionActual != 3) {
                ...
                if (Parametro == 1) { $.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + MarcadorCentroEventual.getPosition().lat() + ",+" + MarcadorCentroEventual.getPosition().lng() + "&key=AIzaSyAsHw9hONULPUhpzrgWzy4MOEd0h6a_UD4", GeolocalizacionCentroEventual); }
    ...
    if (ProyeccionActual != 3) {
        ...
        if (Parametro == 1) { $.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + PinCentro.getPosition().lat() + ",+" + PinCentro.getPosition().lng() + "&key=AIzaSyAsHw9hONULPUhpzrgWzy4MOEd0h6a_UD4", GeolocalizacionCentroElipse); }
...
function DetectarInterseccion(Parametro) {
    ...
    if (PoligonosJTS.length == 2) {
        if (InterceptanDosPoligonos(Proyecciones[0].Elipse, Proyecciones[1].Elipse) == "Si") {
            ...
            if (Parametro == 1) { MapaAImagen(0, 1); { $.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + CentroInterseccion.getPosition().lat() + ",+" + CentroInterseccion.getPosition().lng() + "&key=AIzaSyAsHw9hONULPUhpzrgWzy4MOEd0h6a_UD4", GeolocalizacionCentroInterseccion); } }
        }
    }
...
function MapaAImagen(NProyeccion, Parametro) {
    if (Parametro == 0) {
        if (NProyeccion == 0 || NProyeccion == 1 | NProyeccion == 2) {
            ...
            MapaEstatico += "&key=AIzaSyAt-2vlOp9uyOc8vvUlLi7TiJur1Lix-vQ"
        ...
        for (J = 0; J <= 2; J = J + 1) {
            if (Proyecciones[J].ElipseDibujada) {
                ...
                Documento.autoTable(Columnas, Filas,
                    {
                        theme: "striped",
                        styles: { overflow: 'linebreak', lineWidth: 0.01, halign: 'center', fillcolor: [199, 0, 57] },
                        margin: { top: 5.7 },
                        headerStyles:
                        {
                            lineWidth: 0,
